...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or gkc.conf setting gen)\n"
...
UniValue posminer(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            "posminer (on/off)\n"
            "\nGet/Set pos miner status.\n");

	string result;
	if(params.size() > 0){
		string str = params[0].get_str();
		fMinePosBlock = (str == "off" || str == "false" || str=="0") ? false : true;
	}

	result = strprintf("posminer=%s",fMinePosBlock?"on":"off");
	return result;
}
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "GKC is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "GKC is downloading blocks...");
...
// Release the wallet and main lock while waiting
	...
	BlockHeight nextHeight = chainActive.Tip()->nHeight + 1;
    ...
    // Update nTime
    nextHeight = pindexPrev->nHeight + 1;
    if (crp::IsPowHeight(nextHeight))
...
UniValue submitblock(const UniValue& params, bool fHelp)
{
    ...
    CValidationState state;
	BlockHeight newHeight = chainActive.Height() + 1;
    if (crp::IsPowHeight(newHeight)) {
