...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    const CTransaction& txNew = (crp::IsPosHeight(nBlockHeight) ? block.vtx[1] : block.vtx[0]);
...
#ifdef DPOS
			txNew.vout[i].SetMasternodeReward();
#endif
...
#ifdef DPOS
			txNew.vout[1].SetMasternodeReward();
#endif
...
void CMasternodePayments::AppendPosBlockPayee(BlockHeight newHeight, CAmount reward, std::vector<CTxOut>& vout){
	CScript payee;
	bool hasPayment = true;
	if (!GetBlockPayee(newHeight, payee)) {
		//no masternode detected
		CMasternode* winningNode = mnodeman.GetCurrentMasterNode(1);
		if (winningNode) {
			payee = GetScriptForDestination(winningNode->pubKeyCollateralAddress.GetID());
		} else {
			LogPrint("masternode","AppendPosBlockPayee | Failed to detect masternode to pay\n");
			hasPayment = false;
		}
	}
	if(hasPayment){
		CTxOut txout(reward,payee);
		txout.SetMasternodeReward();
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight, 0, nMasternode_Drift_Count);
