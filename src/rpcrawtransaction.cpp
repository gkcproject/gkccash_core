...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        out.push_back(Pair("value", ValueFromAmount(txout.nValue).write()));
        out.push_back(Pair("n", (int64_t)i));
#ifdef DPOS
		out.push_back(Pair("type", txout.GetTypeString()));
#endif
        UniValue o(UniValue::VOBJ);
        ScriptPubKeyToJSON(txout.scriptPubKey, o, true);
        out.push_back(Pair("scriptPubKey", o));
		if(txout.scriptPubKey.HasOpCreate())
			out.push_back(Pair("contract_address", CreateContractAddress(tx.GetHash(),i)));
        vout.push_back(out);
    }
    entry.push_back(Pair("vout", vout));
    entry.push_back(Pair("type", tx.GetTypeString()));
    entry.push_back(Pair("agentid", tx.agentid.GetHex()));
    if (hashBlock != 0) {
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of gkc addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) gkc address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the gkc address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid GKC address: ") + input.get_str());
	...
	LogPrintf("DPOS listunspent | vecOutputs.size=%u\n",vecOutputs.size());
    BOOST_FOREACH (const COutput& out, vecOutputs) {
        if (out.nDepth < nMinDepth || out.nDepth > nMaxDepth)
        {
			LogPrintf("DPOS listunspent | out=%s\n",out.ToString());
            continue;
        }
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the gkc address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) gkc address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue getkeyid(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
                "getkeyid \"address\"\n"
                "\nConverts a base58 pubkeyhash address to a hex string for use as KeyID.\n"
                "\nArguments:\n"
                "1. \"address\"      (string, required) The base58 address\n"
                "\nResult:\n"
                "\"keyid\"      (string) The raw hex KeyID\n"
                "\nExamples:\n" +
                HelpExampleCli("gethexaddress", "\"address\"") +
                HelpExampleRpc("gethexaddress", "\"address\""));
