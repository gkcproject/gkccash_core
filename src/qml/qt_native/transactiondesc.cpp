...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
	...
	printf("nCredit:%lu   nDebit:%lu\n", nCredit, nDebit);
	printf("txID:%s \n", rec->getTxID().toStdString().c_str());
	...
	if(rec->type == TransactionRecord::EntrustReward || rec->type == TransactionRecord::Entrust)
	{
		strHTML += "<b>" + tr("Agentid") + ":</b> " + QString::fromStdString(wtx.agentid.ToString()) + "<br>";
	}
    strHTML += "<b>" + tr("Transaction ID") + ":</b> " + rec->getTxID() + "<br>";
    strHTML += "<b>" + tr("Output index") + ":</b> " + QString::number(rec->getOutputIndex()) + "<br>";

    // Message from normal gkc:URI (gkc:XyZ...?message=example)
    ...
    if (wtx.IsCoinBase()) {
        quint32 numBlocksToMaturity = Params().COINBASE_MATURITY(chainActive.Height()) + 1;
