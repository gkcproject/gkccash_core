...
bool CWalletDB::WriteZerocoinMint(const CZerocoinMint& zerocoinMint)
{
	...
	LogPrint("zero","CWalletDB::WriteZerocoinMint | hash=%s, zerocoinMint=%s\n",hash.ToString(),zerocoinMint.ToString());
...
{
	LogPrint("zero","CWalletDB::ListMintedCoins | fUnusedOnly=%d, fMaturedOnly=%d, fUpdateStatus=%d\n",fUnusedOnly,fMaturedOnly,fUpdateStatus);
    ...
    for (;;)
    {
        ...
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND){
			LogPrint("zero","CWalletDB::ListMintedCoins | info | not found zerocoin key\n");
            break;
        }
        else if (ret != 0)
        {
        ...
        ssKey >> strType;
        if (strType != "zerocoin"){
			LogPrint("zero","CWalletDB::ListMintedCoins | type is not zerocoin\n");
            break;
        }

        uint256 value;
        ssKey >> value;
		LogPrint("zero","CWalletDB::ListMintedCoins | value=%s\n",value.ToString());

        CZerocoinMint mint;
        ssValue >> mint;
		LogPrint("zero","CWalletDB::ListMintedCoins | mint=%s\n",mint.ToString());

        if (fUnusedOnly) {
            if (mint.IsUsed()){
				LogPrint("zero","CWalletDB::ListMintedCoins | mint.IsUsed()\n");
                continue;
            }
            ...
            if (ReadZerocoinSpendSerialEntry(mint.GetSerialNumber())) {
				...
				LogPrint("zero","CWalletDB::ListMintedCoins | mint spent\n");
                continue;
            }
        }
        ...
        if (fMaturedOnly || fUpdateStatus) {
            ...
            if (mint.GetHeight() > chainActive.Height() - Params().Zerocoin_MintRequiredConfirmations()) {
				...
				LogPrint("zero","CWalletDB::ListMintedCoins | not mature | MintRequiredConfirmations=%d, mint.GetHeight()=%d, chainActive.Height()=%d\n",Params().Zerocoin_MintRequiredConfirmations(),mint.GetHeight(),chainActive.Height());
                continue;
            }
            ...
            if (fMaturedOnly) {
                ...
                if (chainActive.Height() < mint.GetHeight() + 1){
					LogPrint("zero","CWalletDB::ListMintedCoins | not mature 2 | mint.GetHeight()=%d, chainActive.Height()=%d\n",mint.GetHeight(),chainActive.Height());
                    continue;
               	}
                ...
                if(nMintsAdded < Params().Zerocoin_RequiredAccumulation()){
					//LogPrint("zero","CWalletDB::ListMintedCoins | nMintsAdded(%d) < RequiredAccumulation(%d)\n",nMintsAdded,Params().Zerocoin_RequiredAccumulation());
                    continue;
                }
            }
        }
