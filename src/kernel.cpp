...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval) {
        if (!pindexNext) {
            // Should never happen
            LogPrintf("error | GetKernelStakeModifier | "
            	"hashBlockFrom=%s, pindexFrom->nHeight=%d, nStakeModifierHeight=%d, "
            	"nStakeModifierTime=%lld, pindexFrom->GetBlockTime()=%lld, nStakeModifierSelectionInterval=%d\n",
...
{
    //Gkc will hash in the transaction hash and the index number in order to make sure each hash is unique
    ss << nTimeBlockFrom << prevoutIndex << prevoutHash << nTimeTx;
    return Hash(ss.begin(), ss.end());
}

#ifdef DPOS
bool stakeTargetHit(uint256 hashProofOfStake, int64_t nValueIn, uint256 bnTargetPerCoinDay, int currentBlockHeight, AgentID agentid)
{
	const BlockHeight forkHeight = 27600;
	if(chainActive.Height() > forkHeight){
		nValueIn = 1 * COIN;
	}
	
	BlockHeight newHeight = currentBlockHeight + 1;
	BlockHeight baseHeight = Entrustment::GetInstance().GetBaseHeight(newHeight);
	Agent* agent = Entrustment::GetInstance().GetAgent(baseHeight,agentid);
	int64_t nTotalEntrustAmount = agent ? agent->Amount() : 0;
	if(nTotalEntrustAmount <= 0)
		LogPrint("pos","stakeTargetHit | info | nValueIn=%lld, newHeight=%d, baseHeight=%d, agentid=%s, nTotalEntrustAmount=%lld\n",nValueIn,newHeight,baseHeight,agentid.ToString(),nTotalEntrustAmount);
	
    //get the stake weight - weight is equal to coin amount
    uint256 bnCoinDayWeight = uint256(nValueIn + nTotalEntrustAmount) / 100;
...
bool CheckStakeKernelHash(
	...
	int             currentBlockHeight, AgentID       agentid)
#else
...
#ifdef DPOS
        return stakeTargetHit(hashProofOfStake, nValueIn, bnTargetPerCoinDay, currentBlockHeight, agentid);
#else
...
#ifdef DPOS
        if (!stakeTargetHit(hashProofOfStake, nValueIn, bnTargetPerCoinDay, currentBlockHeight, agentid))
#else
...
#ifdef DPOS
    if (!CheckStakeKernelHash(block.nBits, blockprev, txPrev, txin.prevout, nTime, nInterval, true, hashProofOfStake, fDebug, chainActive.Height(), tx.agentid))
#else
