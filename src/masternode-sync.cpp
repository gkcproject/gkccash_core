...
#ifdef DPOS
	//Allow no block for a long time	
#else
...
{
	//LogPrintf("DPOS CMasternodeSync::GetNextAsset | A RequestedMasternodeAssets=%d\n",RequestedMasternodeAssets);
    ...
    RequestedMasternodeAttempt = 0;
    nAssetSyncStarted = GetTime();
	//LogPrintf("DPOS CMasternodeSync::GetNextAsset | B RequestedMasternodeAssets=%d\n",RequestedMasternodeAssets);
}
...
void CMasternodeSync::Process()
{
    ...
    LogPrint("masternode", "DPOS CMasternodeSync::Process() - tick %d RequestedMasternodeAssets %d\n", tick, RequestedMasternodeAssets);

    if (RequestedMasternodeAssets == MASTERNODE_SYNC_INITIAL) GetNextAsset();
	

    // sporks synced but blockchain is not, wait until we're almost at a recent block to continue
    //LogPrint("masternode", "DPOS CMasternodeSync::Process() | IsBlockchainSynced() %d\n", IsBlockchainSynced());
	...
	//LogPrintf("DPOS CMasternodeSync::Process | vNodes.size=%d\n",vNodes.size());
    BOOST_FOREACH (CNode* pnode, vNodes) {
        if (Params().NetworkID() == CBaseChainParams::REGTEST) {
        ...
        if (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) {
			//LogPrintf("DPOS CMasternodeSync::Process | pnode->HasFulfilledRequest(\"getspork\")=%d\n",pnode->HasFulfilledRequest("getspork"));

			if (pnode->HasFulfilledRequest("getspork")) continue;
			pnode->FulfilledRequest("getspork");

			//LogPrintf("DPOS CMasternodeSync::Process | pnode->PushMessage(\"getsporks\"), RequestedMasternodeAttempt=%d.\n",RequestedMasternodeAttempt);
            pnode->PushMessage("getsporks"); //get current network sporks
            if (RequestedMasternodeAttempt >= 2) GetNextAsset();
            RequestedMasternodeAttempt++;

			//LogPrintf("DPOS CMasternodeSync::Process | (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) RequestedMasternodeAttempt=%d\n", RequestedMasternodeAttempt);
            return;
        }
        if (pnode->nVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                LogPrintf(" DPOS CMasternodeSync::Process() | lastMasternodeList=%lld, RequestedMasternodeAttempt=%d\n", lastMasternodeList, RequestedMasternodeAttempt);
            ...
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_MNW) {
				//LogPrintf("DPOS CMasternodeSync::Process | RequestedMasternodeAssets == MASTERNODE_SYNC_MNW(3), "
				//	"lastMasternodeWinner=%lld, RequestedMasternodeAttempt=%d\n",
